...
def brigo_mercurio_abcd_smooth(atm_matrix_df):
    """
    Ultra-optimized row-by-row Brigo-Mercurio Linear-Exponential (ABCD) smoother.
    Utilizes an exact Analytical Jacobian to force light-speed convergence.
    """
    smoothed_matrix = pd.DataFrame(index=atm_matrix_df.index, columns=atm_matrix_df.columns)
    all_tenors = atm_matrix_df.columns.values.astype(float)
    
    # Core ABCD Formula
    def abcd_formula(tau, a, b, c, d):
        return (a + b * tau) * np.exp(-c * tau) + d

    # Exact Analytical Jacobian for light-speed Trust-Region optimization
    def abcd_jacobian(tau, a, b, c, d):
        exp_term = np.exp(-c * tau)
        grad_a = exp_term
        grad_b = tau * exp_term
        grad_c = -tau * (a + b * tau) * exp_term
        grad_d = np.ones_like(tau)
        return np.column_stack([grad_a, grad_b, grad_c, grad_d])

    # Pre-allocate variables to strip overhead inside the loop
    bounds_tuple = ([ -np.inf, -np.inf, 0.0, 0.0 ], [ np.inf,  np.inf, 5.0, np.inf ])
    p0_guess = [0.01, 0.01, 0.1, 0.01]

    for exp in atm_matrix_df.index:
        # Strip pandas overhead by dropping straight into numpy arrays
        row = atm_matrix_df.loc[exp].dropna()
        X_data = row.index.values.astype(float)
        Z_data = row.values.astype(float)
        
        if len(X_data) < 4:
            smoothed_matrix.loc[exp, X_data] = Z_data
            continue
            
        try:
            # The 'jac' parameter prevents Scipy from numerically guessing the derivatives
            popt, _ = curve_fit(
                abcd_formula, X_data, Z_data, 
                p0=p0_guess, 
                bounds=bounds_tuple,
                jac=abcd_jacobian, 
                method='trf'
            )
            # Unpack the 4 optimized parameters and broadcast across all tenors instantly
            smoothed_matrix.loc[exp] = abcd_formula(all_tenors, *popt)
            
        except RuntimeError:
            print(f"Warning: ABCD fit failed for Expiry {exp}Y. Using raw data.")
            smoothed_matrix.loc[exp] = atm_matrix_df.loc[exp]
            
    return smoothed_matrix.astype(float)
